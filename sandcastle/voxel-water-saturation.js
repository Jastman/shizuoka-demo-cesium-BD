...
(async function () {
  ...
  ui.innerHTML = `
    ...
    <div class="vx-body">
      ...
      <label class="vx-field" for="vx-threshold">
        Visibility threshold: <output id="vx-threshold-value">0.40</output>
        <input id="vx-threshold" type="range" min="0" max="0.7" step="0.01" value="0.4" />
      </label>
      ...
      <details class="vx-details">
        <summary>Sources and production data ask</summary>
        <p><a href="${DATA_CATALOG}" target="_blank" rel="noopener noreferrer">Virtual Shizuoka CC BY 4.0 catalog</a> supplies LP, ALB and MMS source context. The 2025 coastal ALB catalog offers source products; it is <strong>not</strong> a measured voxel product.</p>
        <p><strong>CRS discrepancy:</strong> the public 2025 coastal ALB catalog reports JGD2024 / Japan Plane Rectangular CS VIII. PSS cited EPSG:6677, which is JGD2011 / CS IX (EPSG:6676 is JGD2011 / CS VIII). Confirm intended source CRS, coordinate epoch, zone and transformation before tiling.</p>
        <p><strong>PSS deliverable:</strong> ion-hosted voxelized analytical product; aggregation and tiler settings; confirmed source/target CRS and vertical datum; FLOAT32 attribute schema, units, no-data, timestamps and baseline/current lineage.</p>
        <p>Terrain/bathymetry: Cesium World Bathymetry. City context: PLATEAU 2023 MLIT. No access token is embedded.</p>
      </details>
  ...
  ui.querySelector("#vx-view").addEventListener("change", (event) => {
    ...
    state.threshold = state.view === "change" ? 0.08 : 0.4;
