...
(async function shizuokaFloodResponse() {
  ...
  styles.textContent = `
    ...
    .tour-progress-fill { height: 100%; width: 100%; background: var(--cyan);
    ...
    .legend-water { width: 18px; height: 8px; border-radius: 3px; flex-shrink: 0;
      background: linear-gradient(135deg, #2bd8ff, #087f9c 48%, #1d4ed8);
      box-shadow: 0 0 8px rgba(65,217,255,.55); }
    ...
    .gauge-fill { position: absolute; inset: 0 auto 0 0; height: 100%; width: 100%;
  ...
  panel.innerHTML = `
    ...
    <div class="row">
      <label for="fl-water-surface" id="lbl-water-surface"></label>
      <input id="fl-water-surface" type="checkbox" checked />
    </div>
    <details>
  ...
  legend.innerHTML = `
    ...
    <div class="legend-item"><div class="legend-water"></div><span id="leg-water-surface"></span></div>
  ...
  function applyTranslations() {
    ...
    panel.querySelector("#gauge-label").textContent = `▲ ${t("gaugeLabel")}`;
    ...
    legend.querySelector("#leg-water-surface").textContent = t("legendWaterSurface");
  ...
  const waterSurfaceMaterial = new Cesium.Material({
    fabric: {
      type: "Water",
      uniforms: {
        baseWaterColor: Cesium.Color.fromCssColorString("#087f9c").withAlpha(0.68),
        blendColor: Cesium.Color.fromCssColorString("#1d4ed8").withAlpha(0.28),
        normalMap: Cesium.buildModuleUrl("Assets/Textures/waterNormals.jpg"),
        frequency: 140,
        animationSpeed: reducedMotion ? 0 : 0.035,
        amplitude: 1.35,
  ...
  const inverseWaterSurfaceFrame = Cesium.Matrix4.inverse(
    waterSurfaceFrame, new Cesium.Matrix4()
  );
  ...
  function updateWaterSurfaceTransform(level) {
    ...
    Cesium.Matrix4.fromTranslation(
      new Cesium.Cartesian3(0, 0, offset), waterSurfaceTranslation
    );
    Cesium.Matrix4.multiply(
      waterSurfaceFrame, waterSurfaceTranslation, waterSurfaceModelMatrix
    );
    Cesium.Matrix4.multiply(
      waterSurfaceModelMatrix,
      inverseWaterSurfaceFrame,
      waterSurfacePrimitive.modelMatrix
    );
  }
    ...
    polyline: {
      ...
      material: new Cesium.PolylineGlowMaterialProperty({
        ...
        color: new Cesium.CallbackProperty(
          (time) => risk(modelLevel(time)).color.withAlpha(0.95), false
        ),
      }),
    },
  });

  updateWaterSurfaceTransform(modelLevel(start));
  ...
  panel.querySelector("#fl-water-surface").addEventListener("change", (e) => {
    state.waterSurfaceVisible = e.target.checked;
    waterSurfacePrimitive.show = state.waterSurfaceVisible;
    waterEdgeEntity.show = state.waterSurfaceVisible;
  });
  ...
  tour.querySelector("#tour-toggle").addEventListener("click", () => {
    ...
    if (state.tourPaused) {
      ...
      tourStepStart = Date.now() - (tourProgressPct / 100) * stepDuration;
      tourProgressRaf = requestAnimationFrame(animateTourProgress);
      tourAutoTimer = setTimeout(() => {
        if (!state.tourPaused) goToStep(state.tourStep + 1);
      }, stepDuration * (1 - tourProgressPct / 100));
    }
    tour.querySelector("#tour-toggle").textContent = state.tourPaused ? t("tourResume") : t("tourPause");
  });
  ...
  viewer.clock.onTick.addEventListener((clock) => {
    ...
    const gaugeTrack = panel.querySelector("#gauge-track");
    gaugeTrack.setAttribute("aria-valuenow", level.toFixed(2));
    gaugeTrack.setAttribute("aria-valuetext", `${level.toFixed(2)} m`);
